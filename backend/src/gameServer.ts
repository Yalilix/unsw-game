...
function resumeKillCooldowns(gameInstance: GameInstance): void {
	...
	for (const player of gameInstance.players) {
		if (
			...
			player.lastKillTime =
				now - (30000 - player.pausedCooldownRemaining);
		...
		if (socket) {
			...
			const playersWithNames = visiblePlayers.map((visiblePlayer) => {
				...
				return {
					...
					name:
						roomPlayer?.name ||
						`Player (${visiblePlayer.id.slice(-4)})`,
				};
			});
	...
	io.on("connect", (socket) => {
		...
		socket.on(
			...
			(data: {
				...
				const { roomId, playerToken, isCreator, originalSocketId } =
					data;
				...
				if (room && room.status === "playing") {
					...
					if (player) {
						socket.emit("gameState", {
							...
							lastKillTime: player.lastKillTime,
							playerTasks:
								gameInstance.playerTasks[player.id] || [],
							completedTasks: Array.from(
								gameInstance.completedTasks
							),
							sabotageActive: gameInstance.sabotageActive,
							lastSabotageTime: gameInstance.lastSabotageTime,
							repairLocation: gameInstance.sabotageActive
					...
					if (gameInstance) {
						...
						if (!reconnected) {
							...
							if (!existingPlayer) {
								...
								gameInstance.playerTasks[socket.id] =
									assignedTasks;
								console.log(
		...
		socket.on("startGame", (data: { roomId: string }) => {
			...
			if (result.success) {
				...
				if (room) {
					console.log(
						`[DEBUG] Game started successfully for room ${roomId}`
					);
					console.log(
					...
					const startedGameInstance =
						roomManager.getGameInstance(roomId);
					...
					// Notify all players that game started (while still in room_${roomId})
					io.to(`room_${roomId}`).emit("gameStarted", { roomId });
					...
					if (gameInstance) {
						...
						gameInstance.players.forEach((player) => {
							...
							if (socket) {
								...
								console.log(
									`[DEBUG] Socket not found for player ${player.id}`
								);
							}
						});
					}
		...
		socket.on("reportBody", (data: { bodyId: string }) => {
			...
			const reporter = gameInstance.players.find(
				(p) => p.id === socket.id
			);
			if (!reporter || !reporter.isAlive) return;

			const body = gameInstance.deadBodies.find(
				(b) => b.id === data.bodyId
			);
			if (!body) return;
			...
			const alivePlayers = gameInstance.players
				...
				.map((p) => {
					...
					const name =
						roomPlayer?.name || `Player (${p.id.slice(-4)})`;
					console.log(
						`[DEBUG] Player ${
							p.id
		...
		socket.on("kill", () => {
			...
			if (!killer || !killer.isAlive || killer.role !== "imposter")
				return;
		...
		socket.on("sabotage", async () => {
			...
			const imposter = gameInstance.players.find(
				(p) => p.id === socket.id
			);
			...
			try {
				...
				console.error(
					"[DEBUG] Error loading sabotage questions:",
					error
				);
				socket.emit("error", {
					message: "Failed to load sabotage question",
				});
			}
		});
		...
		socket.on("completeSabotage", async (data: { answer: string }) => {
			...
			try {
				...
				const currentQuestion =
					gameInstance.currentQuestions[socket.id];
				...
				console.error(
					"[DEBUG] Error validating sabotage answer:",
					error
				);
				socket.emit("error", {
					message: "Failed to validate sabotage answer",
				});
			}
		});
		...
		socket.on("attemptRepair", async () => {
			...
			if (distance > TILE_SIZE * 1.5) {
				socket.emit("error", {
					message: "Too far from repair location",
				});
				return;
			}
			...
			try {
				...
				socket.emit("error", {
					message: "Failed to load repair question",
				});
			}
		});
		...
		socket.on("completeRepair", async (data: { answer: string }) => {
			...
			try {
				...
				if (!currentQuestion) {
					socket.emit("error", {
						message: "No active repair question found",
					});
					return;
				}
				...
				socket.emit("error", {
					message: "Failed to validate repair answer",
				});
			}
		});
		...
		socket.on(
			"attemptTask",
			async (data: { taskLocation: TaskLocation }) => {
				const room = roomManager.getRoomByPlayer(socket.id);
				if (!room || room.status !== "playing") return;
				...
				if (distance > TILE_SIZE * 1.5) {
					socket.emit("error", {
						message: "Too far from task location",
					});
					return;
				}
				...
				if (!assignedTask) {
					socket.emit("error", {
						message: "No task assigned at this location",
					});
					return;
				}
				...
				try {
					...
					socket.emit("error", {
						message: "Failed to load task question",
					});
				}
			}
		);
		...
		socket.on(
			...
			async (data: { taskLocation: TaskLocation; answer: string }) => {
				...
				try {
					...
					console.error(
						"[DEBUG] Error validating task answer:",
						error
					);
					socket.emit("error", {
			...
			if (room && room.status === "playing") {
				...
				if (gameRunningTime > 5000) {
					...
					if (result.success) {
						...
						console.log(
							`[DEBUG] Player ${socketId} removed from active game`
						);
