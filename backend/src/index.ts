...
app.post("/api/rooms/join", (req, res) => {
	try {
		...
		// Only check if the room exists and is joinable
		...
		if (room.status === "playing") {
			return res
				.status(400)
				.json({ success: false, error: "Game already in progress" });
		}
		...
		// Success: frontend can now navigate to the room page and connect via socket
		res.json({
			success: true,
