...
class RoomManager {
	...
	joinRoom(
		...
		// Check if player already in room (should never be true, but just in case)
	...
	leaveRoom(socketId: string): boolean {
		...
		console.log(
			`[DEBUG] Player ${socketId} left room ${roomId}. Players left: ${room.players
				.map((p) => p.socketId)
				.join(", ")}`
		);
		...
		if (room.host === socketId && room.players.length > 0) {
			...
			console.log(
				`[DEBUG] Host left room ${roomId}. New host: ${room.host}`
			);
		}
		...
		if (room.players.length === 0 && room.status === "waiting") {
			...
			console.log(
				`[DEBUG] Room ${roomId} deleted because it became empty.`
			);
			return true;
		}
	...
	reconnectPlayerToGame(
		...
		if (!existingPlayer) {
			console.log(
				`[DEBUG] No existing player found with ID ${oldSocketId}`
			);
			console.log(
		...
		if (gameInstance.inputsMap[oldSocketId]) {
			gameInstance.inputsMap[newSocketId] =
				gameInstance.inputsMap[oldSocketId];
	...
	updatePlayerName(
		...
		console.log(
			`[DEBUG] Player name updated: ${socketId} -> "${trimmedName}"`
		);
		console.log(
