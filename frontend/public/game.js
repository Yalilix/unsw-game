...
// Server-assigned colors are now used for consistent display across all clients
...
window.attemptRepair = function () {
	if (
		...
		if (myPlayer) {
			const dx = Math.abs(
				myPlayer.x / TILE_SIZE - gameState.repairLocation.x
			);
			const dy = Math.abs(
				myPlayer.y / TILE_SIZE - gameState.repairLocation.y
			);
...
function updateButtons() {
	...
	if (
		...
		if (gameState.gameStartTime > 0) {
			...
			remainingSabotageCooldown = Math.max(
				initialCooldown,
				sabotageCooldown
			);
		}
...
function loop() {
	...
	for (const [i, player] of players.entries()) {
		// Use server-assigned color for consistent display across all clients
		const auraColor = player.color || "#FF4B4B"; // fallback to red if no color

		// Use player ID for stable animation timing
		const stableIndex = player.id.charCodeAt(0) * 100; // Scale up for animation variation
		...
		const bounceOffset =
			Math.sin(
				currentTime * 0.005 * speedMultiplier + stableIndex * 0.008
			) *
			BOUNCE_INTENSITY *
			movementMultiplier;
		const scaleEffect =
			1 +
			Math.sin(
				currentTime * 0.006 * speedMultiplier + stableIndex * 0.003
			) *
				0.02 *
		...
		if (
			...
			if (isMoving) {
				const rotationAngle =
					Math.sin(
						currentTime * 0.004 * speedMultiplier +
							stableIndex * 0.004
					) * 0.08;
			...
			const squashY =
				1 +
				Math.sin(
					currentTime * 0.01 * speedMultiplier + stableIndex * 0.012
				) *
					0.01 *
					movementMultiplier;
		...
		if (gameState.playerTasks.length === 0) {
			...
			for (const task of gameState.playerTasks) {
				...
				if (canvas.measureText(fullText).width <= maxTextWidth) {
					...
					const locationOnlyWrapped = wrapText(
						location,
						indentedMaxWidth
					);
